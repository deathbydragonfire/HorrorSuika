...
/// inspector and the level author so handle behaviour stays in one place.
/// </summary>
public static class PlayfieldShapeAuthoringGui
{
    ...
    /// <summary>Draws live curve preview and control-point handles for the given definition.</summary>
    public static void DrawSceneGui(PlayfieldShapeDefinition definition, bool mirrorX, MergeItemTierTable tierTable)
    {
        if (definition == null)
        {
            return;
        ...
        DrawPreview(definition, tierTable);
        HandleSegmentInsertion(definition, points, mirrorX);
        DrawPointHandles(definition, points, mirrorX);
    }

    /// <summary>
    /// Fills error and warning lists and reports the mesh cost the baker would produce.
    /// </summary>
    public static void CollectValidation(
        PlayfieldShapeDefinition definition,
        MergeItemTierTable tierTable,
        List<string> errors,
        ...
        out int triangleCount)
    {
        ...
        if (definition == null)
        {
            errors.Add("No playfield shape definition assigned.");
            return;
        }
    ...
    /// <summary>Draws bake-cost, errors, and warnings collected by <see cref="CollectValidation"/>.</summary>
    ...
    /// <summary>Bakes the definition to its default or existing prefab path. Returns the prefab, or null on failure.</summary>
    public static GameObject Bake(PlayfieldShapeDefinition definition, GameObject bakeTargetPrefab)
    {
        if (definition == null)
        {
            return null;
        ...
        string prefabPath = bakeTargetPrefab != null
            ? AssetDatabase.GetAssetPath(bakeTargetPrefab)
    ...
    /// <summary>Installs a baked prefab into the scene host and dirties the open scene.</summary>
    public static void Install(GameObject prefab, PlayfieldShapeHost host, Object sceneContext)
    {
        ...
        Undo.RegisterFullObjectHierarchyUndo(host.gameObject, "Install Playfield Shape");
        host.Apply(prefab);
        if (sceneContext is Component component)
        {
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(component.gameObject.scene);
        }
    }

    /// <summary>Largest radius in the table, or 0 when the table is empty.</summary>
    public static float GetLargestTierRadius(MergeItemTierTable table)
    {
    ...
    private static void DrawPreview(PlayfieldShapeDefinition definition, MergeItemTierTable tierTable)
    {
        Vector2[] inner = definition.BuildSampledOutline();
        if (inner.Length < 2)
        {
    ...
    private static void DrawPointHandles(PlayfieldShapeDefinition definition, List<Vector2> points, bool mirrorX)
    {
    ...
    private static void HandleSegmentInsertion(PlayfieldShapeDefinition definition, List<Vector2> points, bool mirrorX)
    {
        Event current = Event.current;
